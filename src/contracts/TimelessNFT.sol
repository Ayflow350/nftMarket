...
function getOwnedNFTs() external view returns (TransactionStruct[] memory) {
    uint256 balance = balanceOf(msg.sender);
    TransactionStruct[] memory ownedNFTs = new TransactionStruct[](balance);
    uint256 index = 0;

    for (uint256 i = 0; i < balance; i++) {
        uint256 tokenId = tokenOfOwnerByIndex(msg.sender, i);
        if (holderOf[tokenId] == msg.sender) {
            ownedNFTs[index] = minted[tokenId - 1];
            index++;
        }
    }
  ...
  function getAccountTransactions() external view returns (TransactionStruct[] memory) {
    uint256 count = 0;

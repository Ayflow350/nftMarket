...
import { setGlobalState, getGlobalState, setAlert } from './store/mine'
...
const getOwnedNFTs = async (address) => {
  try {
    const contract = await getEtheriumContract()
    const ownedNFTIds = await contract.methods.getOwnedNFTs(address).call()
    const ownedNFTs = []

    for (const id of ownedNFTIds) {
     ...
     console.log()
    setGlobalState('ownedNFTs', ownedNFTs)
  } catch (error) {
